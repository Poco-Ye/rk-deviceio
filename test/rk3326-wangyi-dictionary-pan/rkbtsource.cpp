...
int main(int argc, char *argv[])
{
	...
	int sockfd, i, j, item_cnt, ret;
	char scan_result[2048];
	fd_set fds;
	...
	for (i = 0; i < item_cnt; i++) {
		if (!strncmp(argv[1], bt_command_table[i].cmd, strlen(bt_command_table[i].cmd))) {
			if ((i > 2) && ((argc != 3) || (strlen(argv[2]) != 17))) {
				printf("%s:Invalid argument!\n", PRINT_FLAG_ERR);
				show_help(argv[0]);
				return -1;
			}
	...
	if (!strncmp(argv[1], "init", 4)) {
		RK_shell_exec("pidof rksource_server", ret_buff, 4);
		if (!ret_buff[0]) {
	...
	if (!strncmp(argv[1], "scan", 4)) {
		...
		if (FD_ISSET(sockfd, &fds)) {
			...
			for (j = 0; j < scan_msg->devices_cnt; j++) {
				printf("[RK_BT_SCAN]:%s,-%d,%s;\n", scan_devices->name, scan_devices->rssi, scan_devices->addr);
				scan_devices++;
			}
			if (scan_msg->devices_cnt == 0)
				printf("[RK_BT_SCAN]:SCAN_NULL\n");
		} else {
			printf("%s:Socket recv timeout!\n", PRINT_FLAG_ERR);
			goto OUT;
		}
	}
