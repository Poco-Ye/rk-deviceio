...
void kissFftStride(kiss_fft_cfg st,const kiss_fft_cpx *fin,kiss_fft_cpx *fout,int in_stride)
{
    if (fin == fout) {
...
void kissFft(kiss_fft_cfg cfg,const kiss_fft_cpx *fin,kiss_fft_cpx *fout)
{
    kissFftStride(cfg,fin,fout,1);
}

void kissFftCleanup(void)
{
    // nothing needed any more
}

int kissFftNextFastSize(int n)
{
    while(1) {
        int m=n;
        while ( (m%2) == 0 ) m/=2;
