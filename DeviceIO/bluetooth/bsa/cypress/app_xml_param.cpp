...
#define APP_XML_ROOT_KEY                "X_CYPRESS_COM_BluetoothAdapter"
...
#define APP_XML_CONF_AUTH_KEY           "auth"
#if (defined(BLE_INCLUDED) && BLE_INCLUDED == TRUE)
#define APP_XML_CONF_LE_AUTH_KEY        "le_auth"
#endif
...
#define APP_XML_DEV_IS_CONNECTED        "is_connected"
...
int app_xml_read_cfg(const char *p_fname, tAPP_XML_CONFIG *p_xml_config)
{
    ...
    if ((fd = open(p_fname, O_RDONLY)) >= 0)
    {
        ...
        file_buffer = (char *)malloc(maxLen + 1);
        if (file_buffer == NULL)
        {
...
int app_xml_write_cfg(const char *p_fname, const tAPP_XML_CONFIG *p_xml_config)
{
    ...
    app_xml_open_tag(fd, APP_XML_CONF_AUTH_KEY, FALSE);
    dprintf(fd, "%d", p_xml_config->auth_req);
    app_xml_close_tag(fd, APP_XML_CONF_AUTH_KEY, FALSE);

#if (defined(BLE_INCLUDED) && BLE_INCLUDED == TRUE)
    app_xml_open_tag(fd, APP_XML_CONF_LE_AUTH_KEY, FALSE);
    dprintf(fd, "%d", p_xml_config->le_auth_req);
    app_xml_close_tag(fd, APP_XML_CONF_LE_AUTH_KEY, FALSE);
#endif
...
int app_xml_write_db(const char *p_fname, const tAPP_XML_REM_DEVICE *p_xml_rem_devices, int devices_max)
{
    ...
    for (index = 0, instance = 0; index < devices_max; index++, p_xml_rem_devices++)
    {
        ...
        if (p_xml_rem_devices->in_use != FALSE)
        {
            ...
            app_xml_open_tag(fd, APP_XML_DEV_IS_CONNECTED, FALSE);
            dprintf(fd, "%d", p_xml_rem_devices->is_connected);
            app_xml_close_tag(fd, APP_XML_DEV_IS_CONNECTED, FALSE);
            ...
            app_xml_open_tag(fd, APP_XML_DEV_LCSRK_CSRK, FALSE);
            app_xml_write_data(fd, p_xml_rem_devices->lcsrk_csrk, sizeof(p_xml_rem_devices->lcsrk_csrk), FALSE);
            app_xml_close_tag(fd, APP_XML_DEV_LCSRK_CSRK, FALSE);
            ...
            app_xml_open_tag(fd, APP_XML_DEV_LENC_LTK, FALSE);
            app_xml_write_data(fd, p_xml_rem_devices->lenc_ltk, sizeof(p_xml_rem_devices->lenc_ltk), FALSE);
            app_xml_close_tag(fd, APP_XML_DEV_LENC_LTK, FALSE);
        ...
        const char *tagName, unsigned len)
{
    ...
    else if (strncmp(tagName, APP_XML_CONF_AUTH_KEY, strlen(
            APP_XML_CONF_AUTH_KEY)) == 0)
    {
        app_xml_param_cb.cfg_tag = CONF_AUTH_KEY;
    }
...
#if (defined(BLE_INCLUDED) && BLE_INCLUDED == TRUE)
    else if (strncmp(tagName, APP_XML_CONF_LE_AUTH_KEY, strlen(
            APP_XML_CONF_LE_AUTH_KEY)) == 0)
    {
        app_xml_param_cb.cfg_tag = CONF_LE_AUTH_KEY;
    }
#endif
}
        ...
        unsigned len, int more)
{
    ...
    case CONF_AUTH_KEY:
        app_xml_param_cb.config_ptr->auth_req =  app_xml_read_value(data, len);
        break;

#if (defined(BLE_INCLUDED) && BLE_INCLUDED == TRUE)
    case CONF_LE_AUTH_KEY:
        app_xml_param_cb.config_ptr->le_auth_req =  app_xml_read_value(data, len);
        break;
#endif
        ...
        const char *tagName, unsigned len)
{
    ...
    else if (strncmp(tagName, APP_XML_DEV_IS_CONNECTED,
            strlen(APP_XML_DEV_IS_CONNECTED)) == 0)
    {
        app_xml_param_cb.dev_tag = DEV_IS_CONNECTED_KEY;
    }
    ...
    else if (strncmp(tagName, APP_XML_DEV_LCSRK_CSRK, strlen(
            APP_XML_DEV_LCSRK_CSRK)) == 0)
    {
        app_xml_param_cb.dev_tag = DEV_LCSRK_CSRK_KEY;
    }
    ...
    else if (strncmp(tagName, APP_XML_DEV_LENC_LTK, strlen(
        ...
        unsigned len, int more)
{
    ...
    case DEV_IS_CONNECTED_KEY:
        devices_db_ptr->is_connected = app_xml_read_value(data, len);
        break;
    ...
    case DEV_LCSRK_CSRK_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->lcsrk_csrk,
                sizeof(devices_db_ptr->lcsrk_csrk), data, len) == FALSE)
        {
            APP_ERROR0("Failed reading instance lcsrk csrk in XML");
        }
        break;
    ...
    case DEV_LENC_LTK_KEY:
        if (app_xml_read_data_length(&devices_db_ptr->lenc_ltk,
                sizeof(devices_db_ptr->lenc_ltk), data, len) == FALSE)
        {
            APP_ERROR0("Failed reading instance lenc ltk in XML");
        }
        break;
    ...
    for (index = 0; index < nb_device_max; index++)
    {
                ...
                p_stored_device_db[index].bd_addr, bd_addr) == 0))
        {
            ...
            case BSA_LE_KEY_LCSRK:
                ...
                memcpy(p_stored_device_db[index].lcsrk_csrk, ble_key.lcsrk_key.csrk,
                        sizeof(p_stored_device_db[index].lcsrk_csrk));
#endif
...
/*******************************************************************************
 **
 ** Function        app_xml_update_connected_state_db
 **
 ** Description     Update link key information for a device
 **
 ** Returns         0 if successful, error code otherwise
 **
 *******************************************************************************/
int app_xml_update_connected_state_db(tAPP_XML_REM_DEVICE *p_stored_device_db,
        int nb_device_max, BD_ADDR bd_addr, BOOLEAN is_connected)
{
    int index;
