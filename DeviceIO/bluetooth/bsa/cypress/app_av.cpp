...
static const char *app_av_display_vendor_command(UINT8 command);
...
/*******************************************************************************
**
** Function         app_av_register_cb
**
** Description      Register AV status notify
**
...
** Function         app_av_register_cb
**
** Description      DeRegister AV status notify
**
** Parameters       Notify callback
**
** Returns          void
...
void app_av_cback(tBSA_AV_EVT event, tBSA_AV_MSG *p_data)
{
    ...
    case BSA_AV_META_MSG_EVT:
        ...
        APP_DEBUG1("    label=%d, pdu=0x%x, company_id=0x%x",
    ...
    case BSA_AV_FEAT_EVT:
        ...
        {
            int x = app_av_rc_send_register_volume_change_notify_vd_command(p_data->rc_feat.rc_handle);
            APP_DEBUG1("app_av_rc_send_register_volume_change_notify_vd_command:%x", x);
        }
        break;
...
int app_av_rc_change_path_meta_response(int index, tBSA_AV_META_MSG_MSG *pMetaMsg)
{
    ...
    if ( pMetaMsg->param.change_path.direction == AVRC_DIR_UP )
    {
        ...
        if(app_av_cur_folder_empty)
        {
            app_av_cur_folder_empty = 0;
        }
    }
    else if (pMetaMsg->param.change_path.direction == AVRC_DIR_DOWN )
    {
        ...
        APP_INFO0("folder UID ");
        if(app_av_check_uid(pMetaMsg->param.change_path.folder_uid) < 0)
        {
            cmd.param.change_path.status = AVRC_STS_NOT_EXIST;
        }
        if(app_av_check_empty_folder(pMetaMsg->param.change_path.folder_uid) < 0)
        {
            cmd.param.change_path.num_items = 0;
            app_av_cur_folder_empty = 1;
        }
    }
...
int app_av_rc_get_item_attr_meta_response(int index, tBSA_AV_META_MSG_MSG *pMetaMsg)
{
    ...
    APP_INFO1("get_item_attr uid_counter:%d",
        pMetaMsg->param.get_item_attrs.uid_counter);
    ...
    cmd.param.get_item_attrs.status = AVRC_STS_NO_ERROR;

    /* This is for PTS test case. app_av only uses 0 as uid_counter.
     * So, if PTS asked the other value, we need to reply "AVRC_STS_UID_CHANGED"
     * */
    if(pMetaMsg->param.get_item_attrs.uid_counter != 0)
    {
        APP_INFO1("app_av uses 0 as uid_counter value: %d", pMetaMsg->param.get_item_attrs.uid_counter);
        cmd.param.get_item_attrs.status = AVRC_STS_UID_CHANGED;
    }
    else
    {
...
int app_av_rc_play_item_meta_response(int index, tBSA_AV_META_MSG_MSG *pMetaMsg)
{
    ...
    {
        rsp_cmd.param.play_item.status = AVRC_STS_NOT_EXIST;
    }
    else
    {
...
int app_av_rc_add_to_now_playing_meta_response(int index, tBSA_AV_META_MSG_MSG *pMetaMsg)
{
    ...
    {
        rcmd.param.add_to_play.status = AVRC_STS_NOT_EXIST;
    }
    else
    {
...
static int app_av_build_notification_response(UINT8 event_id, tBSA_AV_META_RSP_CMD *p_cmd)
{
    ...
    case AVRC_EVT_TRACK_CHANGE:         /* 0x02 */
        ...
        else
        {
            for(i=0;i<8;i++)
            {
                UINT8_TO_BE_STREAM(p, app_av_songs[p_app_av_cb->cur_play]->uid[i]);
            }
        }
        break;
...
{
    if(p_app_av_cb == NULL)
        p_app_av_cb = (tAPP_AV_CB *)malloc(sizeof(tAPP_AV_CB));
 ...
 **
 *******************************************************************************/
int app_init_playlist(const char *path)
{
    tAPP_WAV_FILE_FORMAT wav_format;
    int index;
        ...
        do
        {
            ...
            else
            {
                ...
                if ((newtime.tv_sec - start) > elapsed)
                {
                    ...
                    APP_DEBUG1("Streaming for %ld secs", elapsed);
                }
...
int app_av_init(BOOLEAN boot)
{
    ...
    app_init_playlist("/data/bsa/config/test_files/av");
    if (app_av_cb.soundfile_list_size > 0)
    {
 ...
 **
 *******************************************************************************/
const char *app_av_get_current_cp_desc(void)
{
 ...
 **
 *******************************************************************************/
const char *app_av_get_notification_string(UINT8 command)
{
    switch(command)
    {
...
void app_av_make_no_track_selected(void)
{
    p_app_av_cb->play_count = 0;
}

#if (defined(AVRC_COVER_ART_INCLUDED) && AVRC_COVER_ART_INCLUDED == TRUE)
void app_av_cover_art_enable()
{
    tBSA_AV_CAS_ENABLE enable;
...
tBSA_UID pts_failuid = {0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff, 0xff};
tBSA_UID pts_empty_folder_uid = {0x16, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

static int app_av_check_uid(tBSA_UID uid)
{
    int result;
    APP_DEBUG1("uid:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
        uid[0], uid[1], uid[2], uid[3], uid[4], uid[5], uid[6], uid[7]);
    APP_DEBUG1("pts_failuid:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
        pts_failuid[0], pts_failuid[1], pts_failuid[2], pts_failuid[3],
        pts_failuid[4], pts_failuid[5], pts_failuid[6], pts_failuid[7]);
    result = memcmp(uid, pts_failuid, 8);
    APP_DEBUG1("result : %d ", result);
...
static int app_av_check_empty_folder(tBSA_UID uid)
{
    ...
    APP_DEBUG1("uid:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
        uid[0], uid[1], uid[2], uid[3], uid[4], uid[5], uid[6], uid[7]);
    APP_DEBUG1("pts_failuid:0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x 0x%02x",
        pts_empty_folder_uid[0], pts_empty_folder_uid[1], pts_empty_folder_uid[2],
        pts_empty_folder_uid[3], pts_empty_folder_uid[4], pts_empty_folder_uid[5],
        pts_empty_folder_uid[6], pts_empty_folder_uid[7]);
    result = memcmp(uid, pts_empty_folder_uid, 8);
    APP_DEBUG1("result : %d ", result);
        ...
        if(app_discovery_cb.devs[index].in_use == TRUE) {
            if(strstr((char *)app_discovery_cb.devs[index].device.playrole, "Audio Sink") != NULL
                ...
                APP_INFO1("\tClassOfDevice:%02x:%02x:%02x => %s",
                        ...
                        app_discovery_cb.devs[index].device.class_of_device[1],
                        app_discovery_cb.devs[index].device.class_of_device[2],
                        app_get_cod_string(app_discovery_cb.devs[index].device.class_of_device));
                APP_INFO1("\tRssi:%d", app_discovery_cb.devs[index].device.rssi);
        
                max_rssi = app_discovery_cb.devs[index].device.rssi;
                target_index = index;
            }
        }
    }
