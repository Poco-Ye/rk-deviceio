...
static void app_hs_process_ciev_msg(BD_ADDR bd_addr, char *msg, UINT16 handle)
{
    if(!msg)
        return;
...
void app_hs_cback(tBSA_HS_EVT event, tBSA_HS_MSG *p_data)
{
    ...
    case BSA_HS_CIEV_EVT:                /* CIEV event */
        ...
        app_hs_process_ciev_msg(p_conn->connected_bd_addr, buf, p_conn->handle);
        break;
