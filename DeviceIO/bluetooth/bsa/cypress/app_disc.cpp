 ...
 **
 *******************************************************************************/
static const char *app_disc_get_uuid16_desc(UINT16 uuid16)
{
    int index;
 ...
 **
 *******************************************************************************/
static void app_disc_parse_eir_uuid16(const char *p_type, UINT8 *p_eir, UINT8 data_length, UINT8 *playrole)
{
    UINT16 uuid16;
    const char *p_desc;
    ...
    while (data_length >= sizeof(UINT16))
    {
        ...
        if (p_desc != NULL)
        {
            ...
            if (playrole) {
                if(uuid16 == 0x110A || uuid16 == 0x110B) {
                    if(strcmp((char *)playrole, "Unknow") == 0) {
                        memcpy((char *)playrole, p_desc, strlen(p_desc));
                    } else {
                        strcat((char *)playrole, " | ");
                        strcat((char *)playrole, p_desc);
                    }
                }
            }
        }
        ...
        {
            APP_INFO1("\t        0x%04X [Unknown]", uuid16);
        }
    }
}
 ...
 **
 *******************************************************************************/
static void app_disc_parse_eir_uuid32(const char *p_type, UINT8 *p_eir, UINT8 data_length)
{
    UINT32 uuid32;
 ...
 **
 *******************************************************************************/
static void app_disc_parse_eir_uuid128(const char *p_type, UINT8 *p_eir, UINT8 data_length)
{
    char buffer[200];
...
void app_disc_parse_eir(UINT8 *p_eir, UINT8 *playrole)
{
    ...
    while(1)
    {
        ...
        case HCI_EIR_MORE_16BITS_UUID_TYPE:
            app_disc_parse_eir_uuid16("Incomplete Service", p, eir_length, playrole);
            break;
        case HCI_EIR_COMPLETE_16BITS_UUID_TYPE:
            app_disc_parse_eir_uuid16("Complete Service", p, eir_length, playrole);
            break;
        ...
        case BTM_BLE_AD_TYPE_SERVICE_DATA:
            app_disc_parse_eir_uuid16("BLE Service", p, eir_length, NULL);
            break;
 ...
 **
 *******************************************************************************/
const char *app_disc_device_type_desc(UINT8 device_type)
{
    switch(device_type)
    {
 ...
 **
 *******************************************************************************/
const char *app_disc_inquiry_type_desc(UINT8 device_type)
{
    switch(device_type)
    {
 ...
 **
 *******************************************************************************/
const char *app_disc_address_type_desc(UINT8 device_type)
{
    switch(device_type)
    {
 ...
 **
 ** Description      Start BR/EDR Device discovery
 **
 ** Returns          int
 **
 *******************************************************************************/
...
/*******************************************************************************
 **
 ** Function         app_disc_start_ble_skip_name
 **
 ** Description      Start BLE Device discovery
 **                  Skip name request
 **
 ...
 **
 *******************************************************************************/
int app_disc_start_ble_skip_name(tBSA_DISC_CBACK *p_custom_disc_cback)
{
    int status;
    tBSA_DISC_START disc_start_param;
