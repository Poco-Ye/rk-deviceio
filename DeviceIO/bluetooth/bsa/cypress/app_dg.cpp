...
typedef struct
{
    ...
    BOOLEAN loopback;
    BOOLEAN disc_get_srv_list_start;  /* flag for discovery and get service list */
    UINT8 disc_index;   /* index for get service device */
} tAPP_DG_CB;
...
const char *app_dg_get_srv_desc(tBSA_SERVICE_ID service);
...
void app_dg_get_sdp_evt(tBSA_DG_MSG *p_data);
...
void app_dg_cback(tBSA_DG_EVT event, tBSA_DG_MSG *p_data)
{
    ...
    case BSA_DG_GET_SDP_EVT:
        app_dg_get_sdp_evt(p_data);
        break;
...
/*******************************************************************************
 **
 ** Function         app_dg_get_sdp_evt
 **
 ** Description     handle get sdp event from DG callback function
 **
 ** Parameters
 **
 ** Returns          void
...
void app_dg_get_sdp_evt(tBSA_DG_MSG *p_data)
{
    ...
    APP_INFO1("BSA_DG_GET_SDP_EVT status:%d", p_data->sdp.status);
    APP_INFO1("\tBdaddr %02x:%02x:%02x:%02x:%02x:%02x",
            p_data->sdp.bd_addr[0], p_data->sdp.bd_addr[1],
            p_data->sdp.bd_addr[2], p_data->sdp.bd_addr[3],
            p_data->sdp.bd_addr[4], p_data->sdp.bd_addr[5]);

    APP_INFO1("\t Protocol UUID:0x%x Found:%d Name:%s SCN:%d",
            p_data->sdp.protocol_uuid,
            p_data->sdp.found,
            p_data->sdp.name,
            p_data->sdp.scn);

    if(p_data->sdp.attrs.num_of_uuid)
    {
        APP_INFO1("\t Total num of attrs : %d", p_data->sdp.attrs.num_of_uuid);

        for (i = 0; i < p_data->sdp.attrs.num_of_uuid ; i++)
        {
            if(p_data->sdp.attrs.uuid[i].len == 2)
            {
                APP_INFO1("\t UUID 16bit 0x%x", p_data->sdp.attrs.uuid[i].uu.uuid16);
            }
            else if(p_data->sdp.attrs.uuid[i].len == 16)
            {
                APP_DUMP("UUID 128bit ", p_data->sdp.attrs.uuid[i].uu.uuid128,
                        p_data->sdp.attrs.uuid[i].len);
            }
        }
    }

    if(p_data->sdp.complete == TRUE)
    {
        APP_DEBUG0("BSA_DG_GET_SDP_EVT complete!");
        /* check discovery and get service list was triggered */
        if(app_dg_cb.disc_get_srv_list_start)
        {
            app_dg_cb.disc_index++;
            for(i = app_dg_cb.disc_index ; i < APP_DISC_NB_DEVICES ; i++)
            {
                if (app_discovery_cb.devs[i].in_use != FALSE)
                {
                    app_dg_cb.disc_index = i;
                    APP_DEBUG1("get service list from device:%d",
                                        app_dg_cb.disc_index);
                    app_dg_get_service_list_with_dev(app_dg_cb.disc_index);
                    break;
                }
            }
            if(i == APP_DISC_NB_DEVICES)
            {
                app_dg_cb.disc_index = 0;
                app_dg_cb.disc_get_srv_list_start = FALSE;
                APP_DEBUG0("Discovery and get service list Finished!!");
            }
        }
    }
    else
    {
        APP_DEBUG0("BSA_DG_GET_SDP_EVT on going!");
    }
}
...
/*******************************************************************************
 **
 ** Function         app_dg_shutdown_all
 **
 ** Description      Example of function to stop all SPP server
 **
 ** Returns          int
 **
 *******************************************************************************/
 void app_dg_shutdown_all()
{
    int index;
    for(index = 0 ; index < APP_DG_NB_CON_MAX ; index++) {
        if (app_dg_cb.connections[index].in_use == TRUE) {
            app_dg_shutdown(index);
        }
    }
}
...
/*******************************************************************************
 **
 ** Function         app_dg_close_all
 **
 ** Description      Example of function to close all DG connection
 **
 ** Returns          int
 **
 *******************************************************************************/
 void app_dg_close_all()
{
    int index;
    for(index = 0 ; index < APP_DG_NB_CON_MAX ; index++) {
        if (app_dg_cb.connections[index].in_use == TRUE) {
...
int app_dg_get_service_name(void)
{
    ...
    BD_ADDR bd_addr;
    UINT8 *p_name;
    tBSA_DG_GET_SDP param;
    int profile;

    APP_DEBUG0("app_dg_get_service_name");
    ...
    BSA_DgGetSdpInit(&param);
    bdcpy(param.bd_addr, bd_addr);

    APP_INFO0("Profile select");
    APP_INFO0("    0 : Seiral Port Profile");
    APP_INFO0("    1 : Dial-up Networking Profile");
    profile = app_get_choice("Select choice");

    if(profile == 0)
    {
        param.uuid16 = UUID_SERVCLASS_SERIAL_PORT;
    }
    else if(profile == 1)
    {
        param.uuid16 = UUID_SERVCLASS_DIALUP_NETWORKING;
    }
    else
    {
        APP_ERROR0("Error!!");
    ...
    APP_INFO1("Opening DG connection to:%s", p_name);
    status = BSA_DgGetSdp(&param);
    if (status != BSA_SUCCESS)
    {
        APP_ERROR1("BSA_DgGetSdp fail:%d", status);
        return -1;
    }
    return 0;
}
...
int app_dg_get_service_list(void)
{
    ...
    APP_DEBUG0("app_dg_get_service_list");
    ...
    param.uuid16 = UUID_SERVCLASS_PUBLIC_BROWSE_GROUP;
...
int app_dg_deinit(void)
{
    int status;
    int index;

    for (index = 0; index < APP_DG_NB_CON_MAX ; index++) {
        status = app_delete_mutex(&app_dg_cb.app_dg_tx_mutex[index]);
        if (status < 0)
            return -1;
    }
...
void app_dg_disc_cback(tBSA_DISC_EVT event, tBSA_DISC_MSG *p_data)
{
    ...
    case BSA_DISC_CMPL_EVT: /* Discovery complete. */
        ...
        if(app_dg_cb.disc_get_srv_list_start)
        {
            APP_INFO0("Get service list");
            for(index = 0 ; index < APP_DISC_NB_DEVICES ; index++)
            {
                if(app_discovery_cb.devs[index].in_use != FALSE)
                {
                    app_dg_cb.disc_index = index;
                    app_dg_get_service_list_with_dev(app_dg_cb.disc_index);
                    break;
                }
            }
    ...
    default:
        APP_ERROR1("app_dg_disc_cback unknown event:%d", event);
        break;
    }
}
...
int app_dg_get_service_list_with_dev(UINT8 device)
{
    ...
    bdcpy(bd_addr, app_discovery_cb.devs[device].device.bd_addr);
    p_name = app_discovery_cb.devs[device].device.name;
...
{
    if(app_dg_connection_index >= APP_DG_NB_CON_MAX) {
        APP_ERROR0("No Connection structure match this DG link");
        return -1;
    }

    if(app_dg_send(app_dg_connection_index, data, len)) {
        APP_ERROR0("send data error");
        return -1;
    }
