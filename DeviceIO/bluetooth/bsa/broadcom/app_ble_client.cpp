...
#define APP_BLE_CLIENT_UUID    0xFEAA
 ...
 * Local functions
 */
static void app_ble_client_state_send(BD_ADDR bd_addr, char *name, RK_BLE_CLIENT_STATE state)
{
    char address[BT_DEVICE_ADDRESS_LEN];

    app_ble_client_ctl.state = state;

    if(!app_ble_client_ctl.state_cb) {
        APP_ERROR0("ble client state cb are not registered\n");
        return;
    }
    ...
    if(name)
        app_ble_client_ctl.state_cb(address, name, state);
    else
        app_ble_client_ctl.state_cb(address, address, state);
}

static void app_ble_client_recv_data_send(tBT_UUID attr_uuid, char *data, int len)
{
    char uuid[UUID_BUF_LEN];

    if(!app_ble_client_ctl.recv_cb)
        return;

    app_ble_client_db_get_uuid(attr_uuid, uuid, UUID_BUF_LEN);
    app_ble_client_ctl.recv_cb(uuid, data, len);
}

static void app_ble_client_send_mtu(UINT16 conn_id, UINT16 mtu)
{
    char address[BT_DEVICE_ADDRESS_LEN];
    int client_num;

    if(!app_ble_client_ctl.mtu_cb) {
        APP_ERROR0("ble client mtu cb are not registered\n");
        return;
    }
    
    client_num = app_ble_client_find_index_by_conn_id(conn_id);
    if(client_num < 0) {
        APP_ERROR1("No find device by conn_id(%d)", conn_id);
        return;
    }

    app_mgr_bd2str(app_ble_cb.ble_client[client_num].server_addr, address, BT_DEVICE_ADDRESS_LEN);
    app_ble_client_ctl.mtu_cb(address, mtu);
}

/*
 * BLE common functions
 */
void app_ble_client_register_state_callback(RK_BLE_CLIENT_STATE_CALLBACK cb)
{
    app_ble_client_ctl.state_cb = cb;
}

void app_ble_client_register_recv_callback(RK_BLE_CLIENT_RECV_CALLBACK cb)
{
    app_ble_client_ctl.recv_cb = cb;
}

void app_ble_client_register_mtu_callback(RK_BT_MTU_CALLBACK cb)
{
    app_ble_client_ctl.mtu_cb = cb;
}

RK_BLE_CLIENT_STATE app_ble_client_get_state()
{
    return app_ble_client_ctl.state;
}
...
/*******************************************************************************
 **
 ** Function         app_ble_client_find_index_by_addr
 **
 ** Description      find BLE client index by address 
 **
 ** Parameters       bd_addr: device address
 **
 ** Returns          positive number(include 0) if successful, error code otherwise
 **
 *******************************************************************************/
int app_ble_client_find_index_by_addr(BD_ADDR bd_addr)
{
    int index;

    for(index = 0; index < BSA_BLE_CLIENT_MAX; index++){
        if(!bdcmp(bd_addr, app_ble_cb.ble_client[index].server_addr))
            return index;
    }
...
int app_ble_client_register(UINT16 uuid)
{
    ...
    if(index < 0) {
        APP_ERROR0("maximum number of BLE client already registered");
        return -1;
    }
...
static int app_ble_client_find_dafault_conn()
{
    int client_num;

    for(client_num = 0; client_num < BSA_BLE_CLIENT_MAX ; client_num++) {
        if(app_ble_cb.ble_client[client_num].enabled) {
 ...
 **
 *******************************************************************************/
int app_ble_client_read(char *uuid)
{
    tBSA_STATUS status;
    tBSA_BLE_CL_READ ble_read_param;
    int ret, client_num;
    tAPP_BLE_CLIENT_DB_CHAR char_id;

    client_num = app_ble_client_find_dafault_conn();
    if((client_num < 0) ||
       (client_num >= BSA_BLE_CLIENT_MAX) ||
       (app_ble_cb.ble_client[client_num].enabled == FALSE)) {
        APP_ERROR1("client %d was not enabled yet", client_num);
        return -1;
    }

    if(app_ble_cb.ble_client[client_num].read_pending) {
        APP_ERROR0("failed : read pending!");
        return -1;
    }

    ret = app_ble_client_db_find_attr(&char_id, app_ble_cb.ble_client[client_num].server_addr, uuid);
    if(ret < 0)
        return -1;

    status = BSA_BleClReadInit(&ble_read_param);
    if (status != BSA_SUCCESS) {
        APP_ERROR1("BSA_BleClReadInit failed status = %d", status);
        return -1;
    }

    ble_read_param.char_id.srvc_id.id.inst_id = 0x00;
    memcpy(&ble_read_param.char_id.srvc_id.id.uuid, &char_id.service_uuid, sizeof(tBT_UUID));
    ble_read_param.char_id.srvc_id.is_primary = char_id.is_primary;

    ble_read_param.char_id.char_id.inst_id = 0x00;
    memcpy(&ble_read_param.char_id.char_id.uuid, &char_id.char_uuid, sizeof(tBT_UUID));
    ...
    if (status != BSA_SUCCESS) {
        APP_ERROR1("BSA_BleClRead failed status = %d", status);
...
int app_ble_client_write(char *char_uuid, char *data, int data_len, BOOLEAN is_descript)
{
    ...
    ret = app_ble_client_db_find_attr(&char_id, app_ble_cb.ble_client[client_num].server_addr, char_uuid);
    if(ret < 0)
        return -1;
    ...
    if(is_descript) {
        ble_write_param.descr_id.char_id.srvc_id.id.inst_id= 0x00;
        memcpy(&ble_write_param.descr_id.char_id.srvc_id.id.uuid, &char_id.service_uuid, sizeof(tBT_UUID));
        ble_write_param.descr_id.char_id.srvc_id.is_primary = char_id.is_primary;

        ble_write_param.descr_id.char_id.char_id.inst_id = 0x00;
        memcpy(&ble_write_param.descr_id.char_id.char_id.uuid, &char_id.char_uuid, sizeof(tBT_UUID));

        ble_write_param.descr_id.descr_id.inst_id = 0x00;
        ...
        ble_write_param.char_id.srvc_id.is_primary = char_id.is_primary;

        ble_write_param.char_id.char_id.inst_id = 0x00;
 ...
 **
 *******************************************************************************/
int app_ble_client_register_notification(char *uuid)
{
    tBSA_STATUS status;
    ...
    memcpy(&ble_notireg_param.notification_id.srvc_id.id.uuid, &char_id.service_uuid, sizeof(tBT_UUID));
    memcpy(&ble_notireg_param.notification_id.char_id.uuid, &char_id.char_uuid, sizeof(tBT_UUID));
    ble_notireg_param.notification_id.srvc_id.id.inst_id = 0x00;
    ble_notireg_param.notification_id.srvc_id.is_primary = char_id.is_primary;
    ble_notireg_param.notification_id.char_id.inst_id = 0x00;
 ...
 **
 *******************************************************************************/
int app_ble_client_open(BD_ADDR bd_addr)
{
    tBSA_STATUS status;
    ...
    if(client_num >= BSA_BLE_CLIENT_MAX) {
        APP_ERROR0("not find available client");
        return -1;
    }
    ...
    //Direct connection:TRUE, Background connection:FALSE
 ...
 **
 *******************************************************************************/
int app_ble_client_close(BD_ADDR bd_addr)
{
    tBSA_STATUS status;
    tBSA_BLE_CL_CLOSE ble_close_param;
    int client_num;

    client_num = app_ble_client_find_index_by_addr(bd_addr);
...
int app_ble_client_close_all()
{
    ...
    for(index = 0; index < BSA_BLE_CLIENT_MAX; index++) {
        if(app_ble_cb.ble_client[index].enabled &&
            ...
            if(app_ble_client_close(app_ble_cb.ble_client[index].server_addr) < 0){
                APP_ERROR1("close connection(%d) failed", index);
                return -1;
            }
        }
    }
 ...
 **
 *******************************************************************************/
int app_ble_client_unpair(BD_ADDR bd_addr)
{
    tBSA_STATUS status;
...
int app_ble_client_remove_device_info(BD_ADDR bd_addr)
{
    tBSA_STATUS status;
    tBSA_SEC_REMOVE_DEV sec_remove_dev;
    tBSA_BLE_CL_CLOSE ble_close_param;
    ...
    APP_INFO0("app_ble_client_remove_device_info");
 ...
 **
 *******************************************************************************/
int app_ble_client_deregister_notification(char *uuid)
{
    tBSA_STATUS status;
    ...
    memcpy(&ble_notidereg_param.notification_id.srvc_id.id.uuid, &char_id.service_uuid, sizeof(tBT_UUID));
    memcpy(&ble_notidereg_param.notification_id.char_id.uuid, &char_id.char_uuid, sizeof(tBT_UUID));
    ble_notidereg_param.notification_id.srvc_id.id.inst_id = 0x00;
    ble_notidereg_param.notification_id.srvc_id.is_primary = char_id.is_primary;
    ble_notidereg_param.notification_id.char_id.inst_id = 0x00;
    bdcpy(ble_notidereg_param.bd_addr, app_ble_cb.ble_client[client_num].server_addr);
    ble_notidereg_param.client_if = app_ble_cb.ble_client[client_num].client_if;
...
static int app_ble_client_mtu_req(UINT16 conn_id)
{
    ...
    if(status != BSA_SUCCESS) {
        APP_ERROR1("BSA_BleClCfgMtuInit failed status:%d", status);
        return -1;
    }

    cfg_mtu.conn_id = conn_id;
    cfg_mtu.mtu = BSA_BLE_CL_WRITE_MAX + BT_ATT_HEADER_LEN;

    status = BSA_BleClCfgMtu(&cfg_mtu);
    if(status != BSA_SUCCESS) {
        APP_ERROR1("BSA_BleClCfgMtu mtu(%d) failed status:%d", cfg_mtu.mtu, status);
        return -1;
    }
...
void app_ble_client_profile_cback(tBSA_BLE_EVT event,  tBSA_BLE_MSG *p_data)
{
    ...
    switch (event)
    {
        ...
        case BSA_BLE_CL_OPEN_EVT:
            ...
            if (p_data->cli_open.status == BSA_SUCCESS)
            {
                ...
                APP_DEBUG1("+++++ %02X:%02X:%02X:%02X:%02X:%02X",
                      ...
                      app_ble_cb.ble_client[client_num].server_addr[3],
                      app_ble_cb.ble_client[client_num].server_addr[4],
                      app_ble_cb.ble_client[client_num].server_addr[5]);
                ...
                app_ble_client_state_send(p_data->cli_open.bd_addr, app_discovery_cb.devs[index].device.name, RK_BLE_CLIENT_STATE_CONNECT);
                app_ble_client_mtu_req(p_data->cli_open.conn_id);
            }
            break;
        ...
        case BSA_BLE_CL_SEARCH_RES_EVT:
            ...
            {
                        scru_dump_hex((char*) p_data->cli_search_res.service_uuid.id.uuid.uu.uuid128, "buffer UUID", 16, TRACE_LAYER_NONE, TRACE_TYPE_DEBUG);
            }
        ...
        case BSA_BLE_CL_READ_EVT:
            ...
            if(p_data->cli_read.status == BSA_SUCCESS)
            {
                ...
                app_ble_client_recv_data_send(p_data->cli_read.char_id.uuid, p_data->cli_read.value, p_data->cli_read.len);
            }
            else
            {
        ...
        case BSA_BLE_CL_CLOSE_EVT:
            ...
            //app_ble_client_remove_device_info(p_data->cli_close.remote_bda);
        ...
        case BSA_BLE_CL_NOTIF_EVT:
            ...
            app_ble_client_recv_data_send(p_data->cli_notif.char_id.char_id.uuid, p_data->cli_notif.value, p_data->cli_notif.len);
        ...
        case BSA_BLE_CL_CFG_MTU_EVT:
            ...
            APP_DEBUG1("status:0x%x, conn_id:%d, mtu:%d", p_data->cli_cfg_mtu.status,
                ...
                app_ble_client_send_mtu(p_data->cli_cfg_mtu.conn_id, p_data->cli_cfg_mtu.mtu);
            break;
...
void app_ble_client_handle_read_evt_ex(tBSA_BLE_CL_READ_MSG *cli_read, FP_DISP_FUNC pAppDispFunc)
{
    ...
    default:
        if(cli_read->char_id.uuid.len == LEN_UUID_16) {
            APP_DEBUG1("char_uuid:0x%04x", cli_read->char_id.uuid.uu.uuid16);
        } else if(cli_read->char_id.uuid.len == LEN_UUID_128) {
            for (int i = 0; i < MAX_UUID_SIZE; i++)
                printf("0x%x ", cli_read->char_id.uuid.uu.uuid128[i]);
            printf("\n");
        }

        APP_DEBUG1("descr_uuid:0x%04x", cli_read->descr_type.uuid.uu.uuid16);
        APP_DEBUG1("Read len = %d", cli_read->len);
...
int app_ble_client_start()
{
    ...
    if(app_ble_client_register(APP_BLE_CLIENT_UUID) < 0) {
        APP_ERROR0("Couldn't Register BLE Client, exiting");
        return -1;
    }
...
void app_ble_client_stop()
{
    app_ble_client_close_all();
