...
void obex_disconnect(int argc, char *btaddr)
{
	...
	if (argc > 1)
		proxy = g_dbus_proxy_lookup(sessions, NULL, btaddr,
...
void obex_get_pbap_pb(char *dir_name, char *dir_file)
{
	...
	memset(pbap_save_filename, 0, 64);
...
static void cmd_cp_pbap_vcf(void)
{
	const char *path = g_dbus_proxy_get_path(default_session);
	GDBusProxy *proxy;
	char *str = "/data/pbap.vcf";

	if (pbap_save_filename[0])
		str = pbap_save_filename;

	bt_shell_printf("pbap_save_filename: %s\n", pbap_save_filename);
...
void *obex_main_thread(void)
{
	...
	obex_main_loop = g_main_loop_new(NULL, FALSE);
...
void obex_quit(void)
{
	g_main_loop_quit(obex_main_loop);
}
